...
ACO <- read.table(paste(filepath, 'time', 'ACO.txt', sep='/'),
						header=FALSE,
						col.names=c('instance', 'runtime'))
GEN <- read.table(paste(filepath, 'time', 'GEN.txt', sep='/'),
						header=FALSE,
						col.names=c('instance', 'runtime'))
...
for (i in instance.sets) {
	ACO.group <- ACO[substr(ACO$instance, 0, nchar(i)) == i,]$runtime
	GEN.group <- GEN[substr(GEN$instance, 0, nchar(i)) == i,]$runtime
...
pdf(paste(filepath, 'output', 'plots', "runtime-sets.pdf", sep='/'))
